...
namespace SDK
{
	...
	public:
		...
		virtual void                  DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial, const matrix3x4_t& transform, const CColor &color) = 0; //79
		...
		{
			auto client_state = *reinterpret_cast<uintptr_t*>(uintptr_t(GetModuleHandle("engine.dll")) + 0x588B34);
			if (!client_state)
				return nullptr;
