...
void CVisuals::Draw()
{
	...
	for (int i = 1; i <= 65; i++)
	{
		...
		{
			if (SETTINGS::settings.zeusrange) DrawTaserDist();
			if (!is_teammate)
			{
				if (SETTINGS::settings.box_bool == 1) DrawBox(entity, box_color, pos, top);
				if (SETTINGS::settings.box_bool == 2) DrawBox2(entity, box_color, pos, top);
			...
			{
				if (SETTINGS::settings.boxteam == 1) DrawBox3(entity, box_color, pos, top);
				if (SETTINGS::settings.boxteam == 2) DrawBox4(entity, box_color, pos, top);
		...
		if (!is_teammate)
		{
			if (SETTINGS::settings.fov_bool) DrawFovArrows(entity, CColor(arrow_colour[0], arrow_colour[1], arrow_colour[2], 255));
		}
		else if (is_teammate)
		{
			if (SETTINGS::settings.arrowteam) DrawFovArrows(entity, CColor(arrowteam_colour[0], arrowteam_colour[1], arrowteam_colour[2], 255));
		}
	}
}
		...
		if (INTERFACES::Engine->GetPlayerInfo(i, &pinfo) && entity->GetHealth() > 0) {
			if (local->GetHealth() > 0) {
				for (int t = 0; t < 12; ++t) {
					Vector screenbacktrack[64][12];
					int index = entity->GetIndex();
					if (backtrack_simtime[i][t] && backtrack_simtime[i][t] + 1 > local->GetSimTime()) {
						if (RENDER::WorldToScreen(backtrack_hitbox[index][i][t], screenbacktrack[i][t])) {
							RENDER::DrawFilledRect(screenbacktrack[i][t].x, screenbacktrack[i][t].y, screenbacktrack[i][t].x + 2, screenbacktrack[i][t].y + 2, CColor(SETTINGS::settings.chamsbackcolor));
						}
					}
				}
			}
...
void CVisuals::FreestandingIndicator()
{
	auto local_player = INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer());
	...
	{
		if (SETTINGS::settings.aa_mode == 8)
		{
			...
			bool is_standing_local = local_player->GetVelocity().Length2D() > 0.0 && local_player->GetFlags();

			if (!is_standing_local & FL_ONGROUND)
			{
				RENDER::DrawF(cW, H * 0.80, FONTS::menu_window_font, true, false, CColor(SETTINGS::settings.FREESTANDINGcolor), "FREESTANDING");
				sprintf_s(bufferlineFakeAngle, "%.1f", GLOBAL::lineFakeAngle);
				RENDER::DrawF(cW, H * 0.80 + 25, FONTS::menu_window_font, true, false, CColor(SETTINGS::settings.FREESTANDINGcolor), bufferlineFakeAngle);
			}
...
void CVisuals::ClientDraw()
{
	...
	//if (SETTINGS::settings.freestandind) FreestandingIndicator();
...
void CVisuals::DrawIndicator()
{
	...
	if (SETTINGS::settings.rifk_arrow)
	{
		...
		float radius = SETTINGS::settings.aasize;
		...
		auto fake_color = CColor(SETTINGS::settings.aacolorreal);
		const auto fake_rot = DEG2RAD(client_viewangles.y - GLOBAL::fake_angles.y - 90);
		draw_arrow(fake_rot, fake_color);

		auto real_color = CColor(SETTINGS::settings.aacolorfake);
...
void CVisuals::DrawBox(SDK::CBaseEntity* entity, CColor color, Vector pos, Vector top)
{
	...
	RENDER::DrawEmptyRect(pos.x - width / 2, top.y, (pos.x - width / 2) + width, top.y + height, CColor(SETTINGS::settings.boxenemy_col));
	RENDER::DrawEmptyRect((pos.x - width / 2) + 1, top.y + 1, (pos.x - width / 2) + width - 1, top.y + height - 1, CColor(SETTINGS::settings.boxenemy_col));
	RENDER::DrawEmptyRect((pos.x - width / 2) - 1, top.y - 1, (pos.x - width / 2) + width + 1, top.y + height + 1, CColor(SETTINGS::settings.boxenemy_col));
}

void CVisuals::DrawBox2(SDK::CBaseEntity* entity, CColor color, Vector pos, Vector top)
{
	float alpha = flPlayerAlpha[entity->GetIndex()];
	int height = (pos.y - top.y), width = height / 2;

	RENDER::DrawOutlineBox(pos.x - width / 2, top.y, width, height, CColor(SETTINGS::settings.boxenemy_col));
}

void CVisuals::DrawBox4(SDK::CBaseEntity* entity, CColor color, Vector pos, Vector top)
{
	float alpha = flPlayerAlpha[entity->GetIndex()];
	int height = (pos.y - top.y), width = height / 2;

	RENDER::DrawOutlineBox(pos.x - width / 2, top.y, width, height, CColor(SETTINGS::settings.boxteam_col));
}

void CVisuals::DrawBox3(SDK::CBaseEntity* entity, CColor color, Vector pos, Vector top)
{
	float alpha = flPlayerAlpha[entity->GetIndex()];
	int height = (pos.y - top.y), width = height / 2;

	RENDER::DrawEmptyRect(pos.x - width / 2, top.y, (pos.x - width / 2) + width, top.y + height, CColor(SETTINGS::settings.boxteam_col));
	RENDER::DrawEmptyRect((pos.x - width / 2) + 1, top.y + 1, (pos.x - width / 2) + width - 1, top.y + height - 1, CColor(SETTINGS::settings.boxteam_col));
	RENDER::DrawEmptyRect((pos.x - width / 2) - 1, top.y - 1, (pos.x - width / 2) + width + 1, top.y + height + 1, CColor(SETTINGS::settings.boxteam_col));
}
...
void CVisuals::DrawBulletBeams()
{
	...
	for (int i = 0; i < Impacts.size(); i++)
	{
		...
		if (current.pPlayer == local_player)
			if (SETTINGS::settings.gradientbullet)
			{
				static float rainbow;
				rainbow += SETTINGS::settings.gradienttracerspeed;
				if (rainbow > 1.f) rainbow = 0.f;
				current.color = CColor::FromHSB(rainbow, 1.f, 1.f);
			}
			else
...
void CVisuals::DrawFovArrows(SDK::CBaseEntity* entity, CColor color)
{
	...
	const auto rot = DEG2RAD(client_viewangles.y - UTILS::CalcAngle(local_player->GetEyePosition(), aimbot->get_hitbox_pos(entity, SDK::HitboxList::HITBOX_PELVIS)).y - 90);
...
void CVisuals::DrawTaserDist()
{
	...
	auto local_player = INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer());
	auto weapon = reinterpret_cast<SDK::CBaseWeapon*>(INTERFACES::ClientEntityList->GetClientEntity(local_player->GetActiveWeaponIndex()));
	if (!weapon || !weapon->GetItemDefenitionIndex() == SDK::WEAPON_TASER)
		return;
		...
		for (float rotation = 0; rotation < (M_PI * 2.0); rotation += step)
		{
			...
			if (RENDER::WorldToScreen(trace.end, scr_pos))
			{
				if (prev_scr_pos.IsValid())
				{
					...
					INTERFACES::Surface->DrawSetColor(CColor(SETTINGS::settings.zeusrangecolor));
