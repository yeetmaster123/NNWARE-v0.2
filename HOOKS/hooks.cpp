...
namespace HOOKS
{
	...
	void AutoRevolver(SDK::CUserCmd* cmd) {
		if (!SETTINGS::settings.auto_revolver)
			return;
	...
	{ 
		...
		if (INTERFACES::Engine->IsConnected() && INTERFACES::Engine->IsInGame())
		{
			...
			{
				ragdoll->SetValue(SETTINGS::settings.radgollslider);
			}
			else {
			...
			prediction->run_prediction(cmd);
			{
				...
				if (SETTINGS::settings.legitbotaimbot)
				{
	...
	void __fastcall HookedDrawModelExecute(void* ecx, void* edx, SDK::IMatRenderContext* context, const SDK::DrawModelState_t& state, const SDK::ModelRenderInfo_t& render_info, matrix3x4_t* matrix)
	{
		...
		if (INTERFACES::Engine->IsConnected() && INTERFACES::Engine->IsInGame())
		{
			...
			if (SETTINGS::settings.wiresleeve_bool == 2)
			{
	...
	void __fastcall HookedSceneEnd(void* ecx, void* edx)
	{
		...
		if (INTERFACES::Engine->IsConnected() && INTERFACES::Engine->IsInGame())
		{
			...
			for (int i = 1; i < 65; i++)
			{
				...
				auto ignorezmaterial2 = SETTINGS::settings.chamstype == 1 ? ignorez : ignorez;
				auto notignorezmaterial2 = SETTINGS::settings.chamstype == 1 ? notignorez : notignorez;
				if (is_local_player)
				{
					switch (SETTINGS::settings.localchams)
					{
						...
						case 4:
							...
							rainbow += SETTINGS::settings.gradientchamsspeed;
				...
				if (is_teammate)
				{
					...
					{
						if (SETTINGS::settings.chamstype == 0)
						{
						...
						else if (SETTINGS::settings.chamstype == 1)
						{
							ignorezmaterial2->ColorModulate(SETTINGS::settings.teaminvis_color);
							INTERFACES::ModelRender->ForcedMaterialOverride(ignorezmaterial2);
							entity->DrawModel(0x1, 255);

							notignorezmaterial2->ColorModulate(SETTINGS::settings.teamvis_color);
							INTERFACES::ModelRender->ForcedMaterialOverride(notignorezmaterial2);
							entity->DrawModel(0x1, 255);
				...
				if (entity && SETTINGS::settings.chams_type == 2)
				{
					...
					{
						ignorezmaterial2->ColorModulate(SETTINGS::settings.imodel_col);
						INTERFACES::ModelRender->ForcedMaterialOverride(ignorezmaterial2);
						entity->DrawModel(0x1, 255);

						notignorezmaterial2->ColorModulate(SETTINGS::settings.vmodel_col);
						INTERFACES::ModelRender->ForcedMaterialOverride(notignorezmaterial2);
						entity->DrawModel(0x1, 255);
			...
			for (auto i = 0; i < INTERFACES::GlowObjManager->GetSize(); i++)
			{
				...
				if (is_local_player && in_tp && SETTINGS::settings.glowlocal)
				{
					...
					{
						static float rainbow;
						rainbow += SETTINGS::settings.gradientglowspeed;
						if (rainbow > 1.f) rainbow = 0.f;
		...
		if (INTERFACES::Engine->IsConnected() && INTERFACES::Engine->IsInGame())
		{
				...
				if (local_player)
				{
					...
					if (!INTERFACES::Input->m_fCameraInThirdPerson)
					{
						...
						if (animstate->m_bInHitGroundAnimation && ground_tick > 1)
							INTERFACES::Input->m_vecCameraOffset = Vector(0.0f, GLOBAL::real_angles.y, GetCorrectDistance(SETTINGS::settings.THIRDPERSONFOV));
						else
							INTERFACES::Input->m_vecCameraOffset = Vector(GLOBAL::real_angles.x, GLOBAL::real_angles.y, GetCorrectDistance(SETTINGS::settings.THIRDPERSONFOV));
						Vector camForward;
			...
			if (!local_player->GetIsScoped())
			{
				if (SETTINGS::settings.vfov_val == 0)
					...
					pSetup->fov = SETTINGS::settings.THIRDPERSONFOV; //110, 90 default
			}
