...
namespace MENU
{
	namespace PPGUI_PP_GUI
	{
		...
		void GroupboxBeginForMain(std::string name, int witdh, int height, int sizex, int sizey, int scroll_size)
		{
			int columns = menu.GetCurrentColumns();
			int width = (menu.GetCurrentEffectiveSize().x - (FRAME_PADDING * (columns - 1))) / columns;
		...
		{
			Vector2D size(RENDER::GetTextSize(menu.GetFont(), name) + Vector2D(35, 11));
