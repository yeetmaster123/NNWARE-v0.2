...
namespace MENU
{
	namespace PPGUI_PP_GUI
	{
		...
		void GroupboxCommand::Draw()
		{
			...
			if (is_top_in_bounds)
			{
				...
				else
				{
					...
					RENDER::DrawFilledRect(position.x, position.y, position.x + size.x, position.y, CColor(30, 30, 30));
		...
		void CheckboxCommand::Draw()
		{
			...
			{
				RENDER::DrawFilledRectOutline(position.x + 0, position.y + 0,
					position.x + info.size.x - 0, position.y + size.y - 0,
		...
		void ComboboxCommand::Draw()
		{
			...
			RENDER::DrawEmptyRect(clamped_position.x - 1, clamped_position.y - 1, clamped_position.x +
				clamped_size.x, clamped_position.y + clamped_size.y, CColor(85, 85, 85, 100));
			...
			RENDER::DrawEmptyRect(info.position.x - 1, info.position.y + info.size.y - 1,
				...
				(min(COMBOBOX_MAX_ITEMS_AT_ONCE, items.size()) + 1)), CColor(85, 85, 85, 100));
		}
		...
		void SliderCommand::Draw()
		{
			...
			if (*variable > slider_min)
			{
				...
				RENDER::DrawFilledRect(position.x + 0, position.y + 0, filled_x_position,
					position.y + size.y - 0, colors[SLIDER_VALUE]);
				RENDER::DrawFilledRect(position.x + 0, position.y + 0, filled_x_position,
					position.y + size.y - 0, CColor(0, 0, 0, 100));
			}
				...
				if (top_in_bounds && bottom_in_bounds)
					if (is_item_selected)
					{
						RENDER::DrawF(draw_position.x + 5, draw_position.y +
							(size_per_item.y * 0.5f), FONTS::numpad_menu_font, false, true,
							colors[VERTICAL_TAB_TEXT_SELECTED], items[i]);
					}
					else
					{
						RENDER::DrawF(draw_position.x + 5, draw_position.y +
